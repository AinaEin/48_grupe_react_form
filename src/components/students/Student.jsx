...
export function Student({ data }) {
  const valideMarks = data.marks.filter(
    (n) => typeof n === "number" && n > 0 && n < 11 && Number.isInteger(n)
  );
  const marksSum = valideMarks.reduce((t, n) => t + n, 0);
  const marksAverage = marksSum / valideMarks.length;
  const marriageStyle =
    style.status + " " + (data.isMarried ? style.married : style.notMarried);
  ...
  return (
    <li className={style.student}>
      ...
      <p className={marriageStyle}>
        Marriage status - is{data.isMarried ? "" : " not"} married
      </p>
